...
class NAFNetBlock(nn.Module):
    def __init__(self, c, DW_Expand=2, FFN_Expand=2, drop_out_rate=0.7):
        super().__init__()
...
class GeneralizedNAFNet(nn.Module):
    def __init__(self, NAFNetBlock: type[nn.Module] = NAFNetBlock, 
                NAFNetDownBlock: type[nn.Module] = NAFNetDownBlock, 
                NAFNetUpBlock: type[nn.Module] = NAFNetUpBlock, 
                in_channels: int = 3, width: int = 16, 
                middle_blk_num: int = 1, 
                enc_blk_nums: tuple[int, ...] = (1, 1, 1, 1), 
                dec_blk_nums: tuple[int, ...] = (1, 1, 1, 1),
                with_relu: bool = False) -> None:
        
        super().__init__()
        self.with_relu = with_relu
        self.intro = nn.Conv2d(in_channels=in_channels, out_channels=width, kernel_size=3, padding=1, stride=1, groups=1, bias=True)
        self.ending = nn.Conv2d(in_channels=width, out_channels=in_channels, kernel_size=3, padding=1, stride=1, groups=1, bias=True)
...
if __name__ == "__main__":
    ...
    model = GeneralizedNAFNet(NAFNetBlock, NAFNetDownBlock, NAFNetUpBlock, in_channels=6, width=16, enc_blk_nums=[1,2,2,2], dec_blk_nums=[2,2,2,1])
    # print(model)
    x = torch.rand(16, 6, 256, 256)
